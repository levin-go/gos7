...
//
func (mb *client) ReadBytes(area int, dbNumber int,start int, size int)(buffer []byte ,err error){
	buffer = make([]byte, size)
	if area == s7areact {
		sbuffer := make([]byte, size*2)
		err = mb.readArea(area, 0, start, size, s7wlcounter, sbuffer)
		if err == nil {
			for c := 0; c < size; c++ {
				buffer[c] = byte(uint(sbuffer[c*2+1])<<8 + uint(sbuffer[c*2]))
			}
		...
		sbuffer := make([]byte, size*2)
		err = mb.readArea(area, 0, start, size, s7wltimer, sbuffer)
		if err == nil {
			for c := 0; c < size; c++ {
				buffer[c] = byte(uint16(sbuffer[c*2+1])<<8 + uint16(sbuffer[c*2]))
			}
		...
		err = mb.readArea(area, dbNumber, start, size, s7wlbyte, buffer)
	}
	return buffer, err
}
func (mb *client) WriteBytes(area int, dbNumber int,start int, buffer []byte)(err error){
	if area == s7areact {
		sbuffer := make([]byte, len(buffer)*2)
		for c := 0; c < len(buffer); c++ {
			sbuffer[c*2+1] = byte((uint(buffer[c]) & uint(0xFF00)) >> 8)
			sbuffer[c*2] = byte(buffer[c] & 0x00FF)
		}
		err = mb.writeArea(s7areact, 0, start, len(buffer), s7wlcounter, sbuffer)
	}else if area == s7areatm{
		sbuffer := make([]byte, len(buffer)*2)
		for c := 0; c < len(buffer); c++ {
			sbuffer[c*2+1] = byte((uint(buffer[c]) & uint(0xFF00)) >> 8)
		...
		err = mb.writeArea(s7areatm, 0, start, len(buffer), s7wltimer, sbuffer)
	}else {
		err = mb.writeArea(area, dbNumber, start, len(buffer), s7wlbyte, buffer)
	}
	return err
}
func (mb *client) WriteBits(area int, dbNumber int,start int, status bool)(err error){
	st := make([]byte, 1)
	if status {
		st[0] = 1
	}else{
	...
	err = mb.writeArea(area, dbNumber, start, 1, s7wlbit, st)
	return err
}
